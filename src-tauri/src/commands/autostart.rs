...
//! All three commands run the schtasks child on a blocking thread: a sync
//! `#[tauri::command]` executes on the main thread, where a ~100ms child
//! process visibly freezes the UI.
...
#[tauri::command]
pub async fn autostart_elevated_status() -> bool {
	#[cfg(windows)]
	{
		tokio::task::spawn_blocking(query_task)
			.await
			.unwrap_or(false)
	}
	#[cfg(not(windows))]
...
#[tauri::command]
pub async fn autostart_elevated_enable() -> Result<(), String> {
	#[cfg(windows)]
	{
		...
		tokio::task::spawn_blocking(create_task)
			.await
...
#[tauri::command]
pub async fn autostart_elevated_disable() -> Result<(), String> {
	#[cfg(windows)]
	{
		...
		let _ = tokio::task::spawn_blocking(delete_task).await;
		Ok(())
	}
	#[cfg(not(windows))]
	{
